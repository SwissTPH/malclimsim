---
title: "Using malclimsim to Infer Parameters of the Climate-driven Dynamical Transmission Model"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Using malclimsim to Infer Parameters of the Climate-driven Dynamical Transmission Model}
...
```{r, echo = FALSE}
path_to_data <- "C:/Users/putnni/switchdrive/Chad/Data/"
rain_file_name <- "chirps_moissala.rds"
temp_file_name <- "era5_moissala.nc"

# Saving the rainfall data locally based on specified path
#save_climate_data(lon = lon, lat = lat, years = years_clim, 
#                  path_to_data = paste0(path_to_data, "climate-data/"),
...
temp_path <- paste0(path_to_data, "climate-data/", temp_file_name)
rain_path <- paste0(path_to_data, "climate-data/", rain_file_name)
...
smc_data_raw <- readxl::read_excel(path_to_SMC)
head(smc_data_raw, n = 0)
...
One last model input is the population size of the region being studied. In the case of Chad, there is must uncertainty about the true population size so we will also include in the model a population scaling factor. This means that the results are not so sensitivity to the chosen population size value.

```{r}
district_pops <- readxl::read_excel(paste0(path_to_data, "model-inputs/district_population_estimates.xlsx")) # 2023 population estimates for select districts of Mandoul
...
param_inputs <- list(
  ...
  pi_p_1 = 0.921, c_p = 1, pi_sp_1 = 0.75 / 0.921, c_sp = 0.12,
  qR = 0.025,
  ...
  b = -log(0.93), R_opt = 0,
  alpha = 0.15,
  T_opt = 27, sigma_LT = 4, sigma_RT = 4,
...
```{r}
plot_time_series(results = results, plot_title = "Malaria in Moissala (2018 to 2023)",
...
```{r}
obs_cases <- readRDS(file.path(path_to_data,"cases-data/MSF-data/opd_district_MOISSALA_2012_2023.rds"))
...
params_to_estimate <- c(lag_R = "lag_R", lag_T = "lag_T", alpha = "alpha",
                        sigma_LT = "sigma_LT", sigma_RT = "sigma_RT",
                        R_opt = "R_opt", k1 = "k1", size_1 = "size_1",
                        eff_SMC = "eff_SMC", s = "s", qR = "qR")
```
...
### Running Inference
```{r}
mcmc_results_out_dir = "C:/Users/putnni/Documents/git-hub-repositories/vignette-storage/malclimsim/"
...
inf_params_stage_2 <- update_inf_stage(results_obj = mcmc_stage_1,
   ...
   shrink = 0.8, stage = "stage2", n_steps = 5000)
...
inf_params_stage_3 <- update_inf_stage(results_obj = mcmc_stage_2,
   ...
   shrink = 0.8, stage = "noadapt", n_steps = 10000)
...
mcmc_stage_3 <- run_stage(proposal_matrix = inf_params_stage_3$proposal_matrix, 
                          ...
                          out_dir = "C:/Users/putnni/Documents/git-hub-repositories/vignette-storage/malclimsim/",
...
## Assessing Model Fit
We will now plot the values predicted by the model to the observed values to assess how well our model is recreating the data. To do this, we sample a certain number of parameter 
...
######################################################
##  Transformation: Scale Incidence by Covariate    ##
######################################################
...
################################################################
##  Extract parameters that maximized posterior probability    #
################################################################
...
# Summarize + plot
sim_data <- summarize_simulation_ci(simulations, variables = c("inc_C_transformed"))
plot_df <- prepare_plot_data(sim_data, obs_cases, "inc_C_transformed", "inc_C")
plot_ppc <- plot_ppc(plot_data = plot_df, ci_data = sim_data, plot_title = "Posterior Predictive Check",
                     data_source = c("Simulated", "Observed"), ylim = NULL, 
                     ylab = "Weekly Number of Cases", xlab = "") 
