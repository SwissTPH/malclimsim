...
head(smc_data_raw, n = 1)


smc_data <- load_clean_smc_data(data = smc_data)
head(smc_data, n = 1)
```
...
param_inputs <- list(
  ...
  b = -log(0.93), R_opt = 0,
  alpha = 0.15,
  T_opt = 27, sigma_LT = 5, sigma_RT = 4.5,
  k1 = 1.8, lag_R = 30, lag_T = 20,
...
start_values_1 <- create_start_values(params_to_estimate, control_params_1,
                                      model = model_1, param_inputs = param_inputs,
...
Here we choose if we are using monthly or weekly data, which age groups we are fitting to, whether or not 
```{r}
